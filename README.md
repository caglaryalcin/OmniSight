...
# override via env: CTID=150 CT_HOSTNAME=mon STORAGE=tank BRIDGE=vmbr1 bash <(curl --proto '=https' --tlsv1.2 -fsSL "https://raw.githubusercontent.com/caglaryalcin/OmniSight/main/scripts/proxmox-lxc.sh?cache=$(date +%s)")
# optional Ubuntu 24.04: DISTRO=ubuntu DISTRO_VERSION=24.04 bash <(curl --proto '=https' --tlsv1.2 -fsSL "https://raw.githubusercontent.com/caglaryalcin/OmniSight/main/scripts/proxmox-lxc.sh?cache=$(date +%s)")
```
...
```bash
bash scripts/install-lxc.sh                 # installs Node 22, clones to /opt/omnisight, sets up systemd
...
If nginx shows `503 Service Temporarily Unavailable`, the ingress could not reach OmniSight. Check the pod, service endpoints and the in-container health endpoints:
