...
function agentRepairCommands(req, agent) {
  ...
  if (role === 'windows') {
    const queryScript = [
      "$result='REPAIR_WINDOWS_AGENT';",
      '$service=Get-Service -Name OmniSightAgent -ErrorAction SilentlyContinue;',
      '$configPath="$env:ProgramData\\OmniSight\\agent.json";',
      "if($service -and $service.Status -eq 'Running' -and (Test-Path -LiteralPath $configPath)){",
      'try{',
      '$config=Get-Content -LiteralPath $configPath -Raw -ErrorAction Stop|ConvertFrom-Json;',
      `if([string]$config.agentId -eq "${id}"){`,
      ...
      "if($ping.report -and [bool]$ping.report.known -and [bool]$ping.report.fresh){$result='HEALTHY'}",
      '}',
      '}catch{',
      '$statusCode=0;',
      'try{if($PSItem.Exception.Response){$statusCode=[int]$PSItem.Exception.Response.StatusCode}}catch{};',
      "if($statusCode -ne 401 -and $statusCode -ne 403){$result='CHECK_FAILED'}",
      '}',
      '};',
      '"RESULT=$result"',
    ].join('');
