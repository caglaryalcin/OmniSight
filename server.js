...
function agentRepairCommands(req, agent) {
  ...
  if (role === 'windows') {
    const queryScript = [
      ...
      "$resultColor = 'White'",
      "$agentHealth = 'UNKNOWN'",
      "$actionMessage = 'Review the diagnostic result.'",
      "if ($service -and $service.Status -eq 'Running') {",
      "  if (-not (Test-Path -LiteralPath $configPath)) {",
      "    $resultMessage = 'RESULT: Agent configuration is missing; use Repair Windows agent.'",
      "    $resultColor = 'Yellow'",
      "    $agentHealth = 'REPAIR REQUIRED'",
      "    $actionMessage = 'Run Repair Windows agent from the dashboard command panel.'",
      '  } else {',
      '    try {',
      '      $config = Get-Content -LiteralPath $configPath -Raw | ConvertFrom-Json',
      '      if ([string]$config.agentId -ne $expectedAgentId) {',
      '        $resultMessage = ("RESULT: Configured agent ID {0} does not match dashboard agent ID {1}; use Repair Windows agent." -f ([string]$config.agentId), $expectedAgentId)',
      "        $resultColor = 'Yellow'",
      "        $agentHealth = 'REPAIR REQUIRED'",
      "        $actionMessage = 'Run Repair Windows agent from the dashboard command panel.'",
      '      } else {',
      ...
      '        if (-not $ping.report -or -not [bool]$ping.report.known) {',
      "          $resultMessage = 'RESULT: OmniSightAgent service is running and dashboard HTTP check passed, but this agent ID has no report; use Repair Windows agent.'",
      "          $resultColor = 'Yellow'",
      "          $agentHealth = 'REPAIR REQUIRED'",
      "          $actionMessage = 'Run Repair Windows agent from the dashboard command panel.'",
      '        } elseif (-not [bool]$ping.report.fresh) {',
      "          $age = if ($null -eq $ping.report.ageSeconds) { 'unknown age' } else { ('{0}s old' -f [int]$ping.report.ageSeconds) }",
      '          $resultMessage = ("RESULT: OmniSightAgent service is running and dashboard HTTP check passed, but the last agent report is stale ({0}); use Repair Windows agent." -f $age)',
      "          $resultColor = 'Yellow'",
      "          $agentHealth = 'STALE / REPAIR REQUIRED'",
      "          $actionMessage = 'Run Repair Windows agent from the dashboard command panel.'",
      '        } else {',
      "          $resultMessage = 'RESULT: OmniSightAgent service is running; dashboard HTTP check passed; latest agent report is fresh.'",
      "          $resultColor = 'Green'",
      "          $agentHealth = 'HEALTHY'",
      "          $actionMessage = 'No repair. Wait up to 30 seconds and refresh the dashboard.'",
      '        }',
      '      }',
      '    } catch {',
      '      $statusCode = 0',
      ...
      '        $resultMessage = ("RESULT: Dashboard returned HTTP {0}; use Repair Windows agent." -f $statusCode)',
      "        $resultColor = 'Yellow'",
      "        $agentHealth = 'REPAIR REQUIRED'",
      "        $actionMessage = 'Run Repair Windows agent from the dashboard command panel.'",
      '      } else {',
      "        $resultMessage = 'RESULT: Dashboard connection/TLS check failed; check the network, OmniSight address or certificate. Do not repair.'",
      "        $resultColor = 'Red'",
      "        $agentHealth = 'CONNECTION CHECK FAILED'",
      "        $actionMessage = 'Do not repair. Check the network, OmniSight address or certificate.'",
      '      }',
      '    }',
      '  }',
      '} elseif ($task) {',
      '  $resultMessage = ("RESULT: Legacy scheduled-task agent detected. State={0}; use Repair Windows agent." -f $task.State)',
      "  $resultColor = 'Yellow'",
      "  $agentHealth = 'REPAIR REQUIRED'",
      "  $actionMessage = 'Run Repair Windows agent from the dashboard command panel.'",
      '} elseif ($service) {',
      '  $resultMessage = ("RESULT: OmniSightAgent service is {0}; use Repair Windows agent." -f $service.Status)',
      "  $resultColor = 'Yellow'",
      "  $agentHealth = 'REPAIR REQUIRED'",
      "  $actionMessage = 'Run Repair Windows agent from the dashboard command panel.'",
      '} else {',
      "  $resultMessage = 'RESULT: OmniSightAgent service and legacy scheduled task are missing; use Repair Windows agent.'",
      "  $resultColor = 'Red'",
      "  $agentHealth = 'MISSING / REPAIR REQUIRED'",
      "  $actionMessage = 'Run Repair Windows agent from the dashboard command panel.'",
      '}',
      'Write-Host ""',
      "if ($service) { Write-Host ('WINDOWS SERVICE PROCESS STATE: {0} (process state only; not agent health)' -f $service.Status) -ForegroundColor Cyan; $service | Format-List Name,DisplayName,Status,StartType; Write-Host 'Service History:'; Write-Host ''; Get-Content \"$env:ProgramData\\OmniSight\\logs\\agent.log\" -Tail 100 -ErrorAction SilentlyContinue } elseif ($task) { Write-Host 'Legacy Scheduled Task:'; Write-Host ''; $task | Format-List TaskName,State }",
      'Write-Host ""',
      "Write-Host ('AGENT HEALTH: {0}' -f $agentHealth) -ForegroundColor $resultColor",
      'Write-Host $resultMessage -ForegroundColor $resultColor',
      "Write-Host ('ACTION: {0}' -f $actionMessage) -ForegroundColor $resultColor",
    ].join('; ');
    const commands = [{
