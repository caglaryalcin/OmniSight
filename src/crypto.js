...
function encryptionEnabled() {
  ...
  const requestedDisable = f === 'false' || f === '0' || f === 'off' || f === 'no';
  const allowPlaintext = ['1', 'true', 'yes', 'on'].includes(String(process.env.OMNISIGHT_ALLOW_PLAINTEXT_SECRETS || '').toLowerCase());
  return !(requestedDisable && allowPlaintext);
}
...
const SENSITIVE_KEYS = new Set([
  ...
  'tokenSecret', 'apiKey', 'apiSecret', 'apiToken', 'accessToken', 'token', 'bearerToken', 'agentToken', 'sshPassword', 'sshKey', 'botToken', 'pingKey', 'webhookUrl',
