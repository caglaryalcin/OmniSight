...
const STATUS_BY_NAME = {
  down: { status: 'down', healthy: false },
  up: { status: 'up', healthy: true },
  pending: { status: 'pending', healthy: true },
  maintenance: { status: 'maintenance', healthy: true },
  unknown: { status: 'unknown', healthy: true },
};
...
function normalizeStatusName(value) {
  ...
  if (typeof value === 'object' && !(value instanceof Date)) {
    for (const key of ['status', 'state', 'code', 'value', 'monitorStatus', 'monitor_status', 'heartbeatStatus', 'heartbeat_status']) {
      const nested = normalizeStatusName(value?.[key]);
  ...
  if (Number.isFinite(numeric) && STATUS[numeric]) return STATUS[numeric].status;
  const s = String(value).trim().toLowerCase();
  if (!s) return null;
  if (['up', 'ok', 'online', 'healthy', 'success', 'passing', 'operational', 'running', 'available'].includes(s)) return 'up';
  if (['down', 'offline', 'fail', 'failed', 'error', 'critical', 'unhealthy', 'unavailable'].includes(s)) return 'down';
  if (['pending', 'paused', 'waiting', 'degraded', 'warning', 'warn'].includes(s)) return 'pending';
...
function statusFromCode(code, monitor = {}) {
  const direct = normalizeStatusName(code);
  if (direct) return STATUS_BY_NAME[direct] || STATUS_BY_NAME.unknown;
  const monitorStatus = normalizeStatusName(monitor.status ?? monitor.state);
  if (monitorStatus) return STATUS_BY_NAME[monitorStatus] || STATUS_BY_NAME.unknown;
  if (monitor.active === false) return STATUS_BY_NAME.maintenance;
  return STATUS_BY_NAME.unknown;
}
...
function chartTimeMs(item = {}) {
  const value = item?.timestamp ?? item?.time ?? item?.date ?? item?.datetime ?? item?.createdDate ?? item?.created_date ?? item?.x;
  if (value === undefined || value === null || value === '') return null;
  ...
  const normalized = normalizeHeartbeatTime(value);
  if (!normalized) return null;
  const ms = new Date(normalized).getTime();
  return Number.isFinite(ms) ? ms : null;
}

function statusBucketActive(value, expectedStatus) {
  if (value === undefined || value === null || value === '') return false;
  if (typeof value === 'boolean') return value;
  const numeric = Number(value);
  if (Number.isFinite(numeric)) return numeric > 0;
  return normalizeStatusName(value) === expectedStatus;
}

function chartItemStatus(item = {}) {
  const directValues = [
    item.status,
    item.state,
    item.monitorStatus,
    item.monitor_status,
    item.heartbeatStatus,
    item.heartbeat_status,
    item.value,
    item.y,
    item.data?.status,
    item.data?.state,
  ...
  if (statusBucketActive(item.down, 'down')) return 'down';
  if (statusBucketActive(item.pending, 'pending')) return 'pending';
  if (statusBucketActive(item.maintenance, 'maintenance')) return 'maintenance';
  if (statusBucketActive(item.up, 'up')) return 'up';
  return null;
}

function chartDataToHeartbeats(list = [], monitor = {}) {
  return replyData(list).map(item => {
    const ms = chartTimeMs(item);
    if (!Number.isFinite(ms)) return null;
    const status = chartItemStatus(item);
