...
function testStaticRegressions() {
  ...
  const takeownLines = windowsInstaller.split(/\r?\n/).filter(line => line.includes('takeown.exe'));
  assert.ok(takeownLines.length > 0 && takeownLines.every(line => !/\/(?:D|R)\b/i.test(line)), 'Windows ownership recovery must not use recursive takeown prompts or locale-specific Y/N answers');
  ...
  const restoredLayoutContext = {};
  vm.runInNewContext(`${dashboard.slice(masonryAppendStart, masonryAppendEnd)}\nfunction stableOverviewIds(ids){ return ids; }\nfunction getOverviewLayout(){ return [['linux','proxmox'],['docker']]; }\n${dashboard.slice(normalizeStart, normalizeEnd)}\nglobalThis.result = normalizeOverviewColumns(['linux','docker','proxmox'], 2, [{dataset:{id:'linux'},offsetHeight:200},{dataset:{id:'docker'},offsetHeight:180},{dataset:{id:'proxmox'},offsetHeight:220}]);`, restoredLayoutContext);
  ...
  assert.ok(windowsQueryArray, 'Windows diagnostic command must be generated as a compact single-line PowerShell statement');
  ...
  assert.ok(windowsQuery.length < 2000, `Windows diagnostic command must stay below the terminal paste limit; got ${windowsQuery.length} characters`);
  assert.ok(windowsQuery.includes('[string]$config.agentId -eq "M4A1-cc94c7b9"') && windowsQuery.endsWith('"RESULT=$result"'), 'Windows diagnostic must validate the selected agent identity and emit only its final RESULT value');
  assert.ok(!/Write-Host|Format-List|Service History|AGENT HEALTH|ACTION:/.test(windowsQuery), 'Windows diagnostic must not print verbose service state or log history');
  if (process.platform === 'win32') {
    ...
    const diagnosticOutput = execFileSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', windowsQuery], { encoding: 'utf8' }).trim();
    assert.match(diagnosticOutput, /^RESULT=(?:HEALTHY|REPAIR_WINDOWS_AGENT|CHECK_FAILED)$/);
  }
